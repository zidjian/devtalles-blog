  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (data.categories && data.categories.length > 0) {
        // Necesitamos obtener los IDs de las categorías
        const categoryIds = categories
          .filter((cat) => data.categories.includes(cat.name))
          .map((cat) => cat.id);

        // Enviar como JSON string para mantener el formato de array
      ...
      alert(
        isNew ? "Post creado exitosamente!" : "Post actualizado exitosamente!"
      );
        ...
        <Card className="bg-black/20 backdrop-blur-sm border-white/10">
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div>
                <label className="block text-white mb-2">Categorías</label>
                <MultiSelect
