...
export default function DashboardPage() {
  ...
  const fetchStats = async (start: string, end: string) => {
    ...
    const response = await authenticatedFetch(`/api/statistics?${params.toString()}`);
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="flex flex-col sm:flex-row gap-4 items-end">
                <div className="flex-1">
                  <label className="block text-white mb-2">Fecha Inicio</label>
                  <input
                    type="date"
                    {...register("startDate")}
                ...
                <div className="flex-1">
                  <label className="block text-white mb-2">Fecha Fin</label>
                  <input
                    type="date"
                    {...register("endDate")}
                    className="w-full rounded-md border border-white/20 bg-white/5 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-white/20"
